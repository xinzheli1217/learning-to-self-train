...
def process_batch_test(input_filename_list, input_label_list, dim_input, batch_sample_num, reshape_with_one=True):
    new_path_list = []
    new_label_list = []
    for k in range(batch_sample_num):
        class_idxs = range(0, FLAGS.way_num)
    ...
    if reshape_with_one:
        img_array = np.array(img_list).reshape([1, FLAGS.way_num*batch_sample_num, dim_input])
        label_array = one_hot(np.array(new_label_list)).reshape([1, FLAGS.way_num*batch_sample_num, -1])
        img_array = np.tile(img_array, (1,repeat_num,1))
        label_array = np.tile(label_array, (1,repeat_num,1))
    else:
        img_array = np.array(img_list).reshape([FLAGS.way_num*batch_sample_num, dim_input])
        label_array = one_hot(np.array(new_label_list)).reshape([FLAGS.way_num*batch_sample_num, -1])
        img_array = np.tile(img_array, (repeat_num, 1))
        label_array = np.tile(label_array, (repeat_num, 1))
    return img_array, label_array

def process_un_batch_new(input_filename_list, input_label_list, dim_input, reshape_with_one=False):
...
def process_un_batch_2_new(input_filename_list, input_label_list, dim_input, reshape_with_one=False):
...
def process_dis_batch_new(input_filename_list, dim_input, reshape_with_one=False):
    ...
    if reshape_with_one:
        img_array = np.array(img_list).reshape([1, 3*FLAGS.nb_ul_samples, dim_input])

    else:
        img_array = np.array(img_list).reshape([3*FLAGS.nb_ul_samples, dim_input])
...
def get_random_labels(reshape_with_one=True):
    random_labels = np.zeros((FLAGS.way_num*FLAGS.nb_ul_samples,), dtype=np.int32)
    for i in range(FLAGS.way_num*FLAGS.nb_ul_samples):
        random_label = np.random.randint(0, FLAGS.way_num)
        random_labels[i] = random_label
    if reshape_with_one:
        random_labels = one_hot(random_labels).reshape([1, FLAGS.way_num * FLAGS.nb_ul_samples, -1])
    else:
        random_labels = one_hot(random_labels).reshape([FLAGS.way_num * FLAGS.nb_ul_samples, -1])
